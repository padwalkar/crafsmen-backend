...
export default function EditServiceType({ openEditServiceDialog, setOpenEditServiceDialog, setLoading, loadList, editRecord }) {
    ...
    const [serviceType, setServiceType] = useState(editRecord.serviceTypeId);
    ...
    const handleSaveClick = async () => {
        ...
        if (simpleValidator.current.allValid()) {
            ...
            setLoading(true);
            axios
                .put(`/updateService?i=${editRecord.serviceId}`, postBody)
                .then(op => {
                    setLoading(false);
                    if (!_.isEmpty(op) && !_.isEmpty(op.data) && op.data.message === 'SERVICE_UPDATED') {
